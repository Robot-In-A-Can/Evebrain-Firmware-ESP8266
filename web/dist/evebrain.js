...
EveBrain.prototype = {
  ...
  connect: function(){
    if(!this.connected && !this.error){
      ...
      this.ws.onerror = function(err){self.handleError(err); console.log(err);} //diff
      this.ws.onclose = function(err){self.handleError(err); console.log(err);} //diff
  ...
  send: function(msg, cb){
    ...
    if(['stop', 'pause', 'resume', 'ping', 'version','gpio_on','gpio_off'].indexOf(msg.cmd) >= 0){
      this.send_msg(msg);
    }else{
